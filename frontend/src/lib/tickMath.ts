// Port of Uniswap TickMath.getSqrtPriceAtTick (Q64.96).

const Q32 = 1n << 32n;

export function getSqrtRatioAtTick(tick: number): bigint {
  const absTick = tick < 0 ? -tick : tick;
  if (absTick > 887272) throw new Error("Invalid tick");
  ...
  if (tick > 0) price = (2n ** 256n - 1n) / price;

  return (price + Q32 - 1n) >> 32n;
}
